...
function createTheHtml(l1,l2,l3,l4,l5,l6,l7,l8,l9,l10,l11,l12,n){
	...
	for(i = 0; i<num; i++)
	{
		result += "<li><a href=single-star.html?id="+ l1[i]+"&title="+replaceSpace(l3)+"&year="+l4+"&director="+l5+"&star="+l6+"&firstRecord="+l7+"&numRecord="+l8+"&sortType="+l9+"&sortOrder="+l10+"&title_i="+l11+"&genre="+replaceSpace(l12)+">"+ l2[i] + "</a></li>";
	}
	result += "</ul></th>";
	return result;
}
...
//    }
    ...
    for (let i = 0; i < resultData.length; i++) {
        ...
        rowHTML += createTheHtml(resultData[i]["star_id"],resultData[i]["movie_star"],replaceSpace(resultData[i]["mtitle"]),resultData[i]['myear'],resultData[i]['mdirector'],resultData[i]['mstar'],resultData[i]['mfirstRecord'],resultData[i]['mnumRecord'],resultData[i]['msortType'],resultData[i]['msortOrder'],resultData[i]["mtitle_i"],resultData[i]["mgenre"],resultData[i]["stars_num"]);
